...
def get_download_folder(output_dir, podcast_title):
    if not output_dir:
        dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'temp_output_dir')
    else:
        dir_path = output_dir
...
def create_filename_and_folders(output_dir, podcast_title, filename):
    ...
    # if not os.path.exists(dir_path):
    #     os.makedirs(dir_path)
    dir_path = get_download_folder(output_dir, podcast_title)
